...
function TaskTableRow({task, page}:{task:Task; page:number}) {
    const { completeTask, fetchTasks } = React.useContext(TaskContext) as TasksContextType;
    const [isEditModalOpen, setIsEditModalOpen] = React.useState(false);
    const [isDeleteModalOpen, setIsDeleteModalOpen] = React.useState(false);
    const closeEditModal = () => {setIsEditModalOpen(false);}
    const closeDeleteModal = () => {setIsDeleteModalOpen(false);}
    const priorityBackGround = (dueDate:string):string=>{
        if(dueDate===null) return "bg-white"
        else{
    ...
    return(
        <tr className={priorityBackGround(task.dueDate)}>
            ...
            <td className="py-4 px-6 border-b border-gray-200 flex gap-4">
                ...
                <ModalEditToDo isOpen={isEditModalOpen} closeModal={closeEditModal} taskToEdit={task} page={page}/>
                ...
                <ModalDeleteToDo isOpen={isDeleteModalOpen} closeModal={closeDeleteModal} taskToDelete={task} page={page}/>
            </td>
        </tr>
    )
}
